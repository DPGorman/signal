...
If no meaningful connections exist, return {"connections": []}`,
      ...
      <div style={{ marginTop: 10 }}>
        {existing.map(r => (
          <div key={r.id} style={{ padding: "10px 14px", background: C.bg, borderLeft: `3px solid ${C.blue}`, marginBottom: 8 }}>
            <div style={{ fontSize: 15, color: C.blue, fontFamily: mono, letterSpacing: "0.1em", marginBottom: 4 }}>
        ...
        <div style={{ display: "flex", gap: 6 }}>
          ...
          {draft.trim() && (
            <button onClick={send}
              style={{ background: C.blue, border: "none", color: C.bg, padding: "7px 12px", fontFamily: mono, fontSize: 15, cursor: "pointer", flexShrink: 0 }}>
              ↵
            </button>
          )}
        </div>
  ...
  const SectionHead = ({ label, onClick }) => (
    <div onClick={onClick}
      style={{ fontSize: 15, color: C.textMuted, fontFamily: mono, letterSpacing: "0.15em", cursor: onClick ? "pointer" : "default", display: "inline-flex", alignItems: "center", gap: 5, transition: "color 0.15s" }}
      onMouseEnter={e => onClick && (e.currentTarget.style.color = C.gold)}
      onMouseLeave={e => (e.currentTarget.style.color = C.textMuted)}>
      {label}{onClick && <span style={{ fontSize: 15 }}>→</span>}
    </div>
  );
    ...
    <div style={{ height: "100vh", background: C.bg, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: sans }}>
      <div style={{ width: 340, display: "flex", flexDirection: "column", gap: 20 }}>
        <div style={{ textAlign: "center", marginBottom: 8 }}>
          ...
          <div style={{ fontSize: 15, color: C.textMuted, fontFamily: mono, letterSpacing: "0.1em" }}>
        ...
        <input
          ...
          style={{ width: "100%", background: C.surface, border: `1px solid ${C.border}`, color: C.textPrimary, padding: "12px 14px", fontSize: 15, fontFamily: sans, outline: "none", borderRadius: 6, boxSizing: "border-box" }}
        />
        <input
        ...
        />
        {authError && (
          <div style={{ fontSize: 15, color: C.red, textAlign: "center", lineHeight: 1.5 }}>{authError}</div>
        )}
        <button
          onClick={() => handleAuth(authScreen)}
          disabled={authLoading || !authEmail || !authPass}
          style={{ width: "100%", background: C.gold, border: "none", color: C.bg, padding: "12px", fontFamily: mono, fontSize: 15, letterSpacing: "0.12em", cursor: authLoading ? "default" : "pointer", borderRadius: 6, opacity: authLoading ? 0.6 : 1 }}>
          {authLoading ? "..." : authScreen === "login" ? "LOG IN →" : "SIGN UP →"}
        </button>
        <div style={{ textAlign: "center" }}>
          <button
            onClick={() => { setAuthScreen(authScreen === "login" ? "signup" : "login"); setAuthError(""); }}
            style={{ background: "none", border: "none", color: C.textMuted, fontSize: 15, cursor: "pointer", fontFamily: sans }}>
  ...
  if (onboarding) return (
    <div style={{ height: "100vh", background: C.bg, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: sans }}>
      <div style={{ width: 380, display: "flex", flexDirection: "column", gap: 20 }}>
        <div style={{ textAlign: "center", marginBottom: 8 }}>
          <div style={{ fontSize: 29, color: C.textPrimary, fontStyle: "italic", letterSpacing: "-0.03em", marginBottom: 6 }}>Signal</div>
          <div style={{ fontSize: 15, color: C.textMuted, fontFamily: mono, letterSpacing: "0.1em" }}>NAME YOUR PROJECT</div>
        </div>
  ...
  const DashboardView = () => (
    <div style={{ flex: 1, overflowY: "auto", overflowX: "hidden", padding: "36px 44px" }}>
      <div style={{ marginBottom: 32 }}>
        ...
        <div style={{ fontSize: 15, color: C.textMuted, fontFamily: mono }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 8, marginBottom: 36 }}>
        {[
          ...
          <div key={s.label} onClick={() => navGo(s.dest)}
            ...
            <div style={{ fontSize: 15, color: C.textPrimary, fontWeight: 500, marginBottom: 2 }}>{s.label}</div>
            <div style={{ fontSize: 15, color: C.textMuted }}>{s.sub}</div>
          </div>
        ))}
      </div>
      <div style={{ marginBottom: 24, background: C.surface, border: `1px solid ${C.border}` }}>
        <div style={{ padding: "14px 18px", borderBottom: `1px solid ${C.border}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <span onClick={() => navGo("library")} style={{ fontSize: 15, color: C.gold, cursor: "pointer", fontFamily: mono }}>VIEW ALL →</span>
        </div>
        {ideas.length === 0
          ? <div style={{ padding: "24px 20px", color: C.textDisabled, fontStyle: "italic", fontSize: 15 }}>No ideas yet.</div>
          : ideas.slice(0, 6).map((idea, idx) => {
              ...
              return (
                <div key={idea.id} onClick={() => navGo("library", idea)}
                  ...
                  <span style={{ fontSize: 15, color: cat.color, marginTop: 2, flexShrink: 0 }}>{cat.icon}</span>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 15, color: C.textSecondary, lineHeight: 1.6, display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden", wordBreak: "break-word" }}>{idea.text}</div>
                    <div style={{ fontSize: 15, color: C.textMuted, fontFamily: mono, marginTop: 3 }}>
      ...
      <div style={{ marginBottom: 24, background: C.surface, border: `1px solid ${C.border}` }}>
        <div style={{ padding: "14px 18px", borderBottom: `1px solid ${C.border}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <span onClick={() => navGo("deliverables")} style={{ fontSize: 15, color: C.gold, cursor: "pointer", fontFamily: mono }}>VIEW ALL →</span>
        </div>
        {pending.length === 0
          ? <div style={{ padding: "24px 20px", color: C.textDisabled, fontStyle: "italic", fontSize: 15 }}>All caught up.</div>
          : pending.slice(0, 5).map((task, idx, arr) => {
              ...
              return (
                <div key={task.id} onClick={() => toggleDeliverable(task.id, task.is_complete)}
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 15, color: C.textSecondary, lineHeight: 1.6, display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden", wordBreak: "break-word" }}>{task.text}</div>
                    <div style={{ fontSize: 15, color: cat.color, fontFamily: mono, marginTop: 3 }}>{cat.icon} {cat.label}</div>
                  </div>
                </div>
              );
            })
          ...
          <div style={{ display: "flex", gap: 14, flexWrap: "wrap" }}>
            {CATEGORIES.filter(cat => ideas.some(i => i.category === cat.id)).map(cat => (
              <span key={cat.id} onClick={() => { setFilterCat(cat.id); navGo("library"); }}
                style={{ fontSize: 15, color: C.textMuted, cursor: "pointer", transition: "color 0.15s" }}
      ...
      {canonDocs.length > 0 && (
        <div style={{ background: C.surface, border: `1px solid ${C.border}`, padding: "16px 18px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
            ...
            <span onClick={() => navGo("canon")} style={{ fontSize: 15, color: C.gold, cursor: "pointer", fontFamily: mono }}>MANAGE →</span>
          </div>
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
            {canonDocs.map(doc => (
              <div key={doc.id} onClick={() => { setActiveDoc(doc); navGo("canon"); }}
                ...
                <div>
                  <div style={{ fontSize: 15, color: doc.is_active ? C.textPrimary : C.textDisabled }}>{doc.title}</div>
                  <div style={{ fontSize: 15, color: C.textMuted, fontFamily: mono }}>{doc.is_active ? "active" : "inactive"}</div>
                </div>
              </div>
            ))}
          </div>
    ...
    <div style={{ flex: 1, overflowY: "auto", padding: "52px 56px" }}>
      <div style={{ maxWidth: 660 }}>
        <div style={{ borderLeft: `3px solid ${C.gold}`, paddingLeft: 20, marginBottom: 48 }}>
          <div style={{ fontSize: 15, color: C.gold, fontFamily: mono, letterSpacing: "0.15em", marginBottom: 10 }}>TODAY'S INVITATION</div>
          <div style={{ fontSize: 19, lineHeight: 1.9, color: C.textMuted, fontStyle: "italic" }}>{todayInvitation}</div>
        </div>
        <div style={{ fontSize: 15, color: C.textMuted, fontFamily: mono, letterSpacing: "0.15em", marginBottom: 8 }}>WHAT'S IN YOUR HEAD RIGHT NOW</div>
        ...
        />
        <div style={{ fontSize: 15, color: C.textMuted, fontFamily: mono, letterSpacing: "0.15em", marginBottom: 8 }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <span style={{ fontSize: 15, color: C.textDisabled, fontFamily: mono }}>⌘ + ENTER</span>
          <button onClick={captureIdea} disabled={isAnalyzing}
            style={{ background: isAnalyzing ? C.surfaceHigh : C.gold, color: isAnalyzing ? C.textMuted : C.bg, border: "none", padding: "12px 32px", fontFamily: mono, fontSize: 15, letterSpacing: "0.1em", cursor: isAnalyzing ? "default" : "pointer" }}>
        ...
        <div style={{ marginTop: 56, paddingTop: 32, borderTop: `1px solid ${C.border}`, display: "flex", gap: 48 }}>
          {[
            ...
            <div key={s.l} onClick={() => navGo(s.dest)} style={{ cursor: "pointer" }}>
              ...
              <div style={{ fontSize: 15, color: C.textMuted, fontFamily: mono, letterSpacing: "0.12em", marginTop: 8 }}>{s.l}</div>
            </div>
          ))}
        </div>
      </div>
                ...
                return (
                  <div style={{ maxWidth: 640 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 22 }}>
                      <span style={{ fontSize: 15, color: cat.color, fontFamily: mono, letterSpacing: "0.1em" }}>{cat.icon} {cat.label.toUpperCase()}</span>
                      {displayIdea.signal_strength >= 4 && <span style={{ fontSize: 15, color: C.gold, fontFamily: mono, border: `1px solid ${C.gold}40`, padding: "2px 10px" }}>HIGH SIGNAL</span>}
                      {searchHighlight && <span onClick={() => setSearchHighlight("")} style={{ fontSize: 15, color: C.gold, fontFamily: mono, border: `1px solid ${C.gold}40`, padding: "2px 10px", cursor: "pointer" }}>✕ CLEAR HIGHLIGHT</span>}
                      <span style={{ flex: 1 }} />
                      <button onClick={() => deleteIdea(displayIdea.id)}
                        style={{ fontSize: 15, color: C.red, background: "transparent", border: `1px solid ${C.border}`, padding: "3px 10px", fontFamily: mono, cursor: "pointer", borderRadius: 4, opacity: 0.6 }}
                        onMouseEnter={e => e.currentTarget.style.opacity = 1}
                        onMouseLeave={e => e.currentTarget.style.opacity = 0.6}>
                        DELETE
                      </button>
                      <span style={{ fontSize: 15, color: C.textDisabled, fontFamily: mono }}>
                    ...
                    {displayIdea.inspiration_question && (
                      <div style={{ marginBottom: 32, padding: "16px 20px", background: C.surfaceHigh, borderLeft: `3px solid ${C.textMuted}` }}>
                        <div style={{ fontSize: 15, color: C.textMuted, fontFamily: mono, letterSpacing: "0.12em", marginBottom: 8 }}>WHY IT FELT IMPORTANT</div>
                    ...
                    {displayIdea.ai_note && (
                      <div style={{ marginBottom: 32 }}>
                        <div style={{ fontSize: 15, color: C.gold, fontFamily: mono, letterSpacing: "0.12em", marginBottom: 10 }}>DRAMATURGICAL ANALYSIS</div>
                        <div style={{ fontSize: 15, color: C.textSecondary, lineHeight: 1.65 }}><Highlight text={displayIdea.ai_note} term={searchHighlight} /></div>
                        <ReplyBox ideaId={displayIdea.id} section="ai_note" />
                      </div>
                    )}
                    {displayIdea.canon_resonance && (
                      <div style={{ marginBottom: 32 }}>
                        <div style={{ fontSize: 15, color: C.purple, fontFamily: mono, letterSpacing: "0.12em", marginBottom: 10 }}>CANON RESONANCE</div>
                    ...
                    {displayIdea.dimensions?.length > 0 && (
                      <div style={{ marginBottom: 32 }}>
                        <div style={{ fontSize: 15, color: C.textMuted, fontFamily: mono, letterSpacing: "0.12em", marginBottom: 12 }}>DIMENSIONS</div>
                        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                          {displayIdea.dimensions.map((d, i) => (
                            <span key={i} style={{ fontSize: 15, color: C.textSecondary, border: `1px solid ${C.border}`, padding: "5px 14px", fontFamily: mono }}>{d.label}</span>
                          ))}
                        </div>
                      </div>
                    )}
                    {ideaDels.length > 0 && (
                      <div style={{ borderTop: `1px solid ${C.border}`, paddingTop: 28 }}>
                        <div style={{ fontSize: 15, color: C.textMuted, fontFamily: mono, letterSpacing: "0.12em", marginBottom: 16 }}>INVITATIONS TO ACTION</div>
                        {ideaDels.map(d => (
      ...
      {showUpload && (
        <div style={{ maxWidth: 500, marginBottom: 32, padding: "20px 24px", background: C.surfaceHigh, border: `1px solid ${C.border}`, borderRadius: 6 }}>
          <input value={canonUpload.title} onChange={e => setCanonUpload(p => ({ ...p, title: e.target.value }))}
            ...
            style={{ ...inputBase, marginBottom: 10, fontSize: 15 }} />
          <select value={canonUpload.type} onChange={e => setCanonUpload(p => ({ ...p, type: e.target.value }))}
            style={{ ...inputBase, marginBottom: 10, fontSize: 15 }}>
            {DOC_TYPES.map(t => <option key={t.id} value={t.id}>{t.label}</option>)}
          </select>
          <div style={{ fontSize: 15, color: C.textMuted, fontFamily: mono, letterSpacing: "0.1em", marginBottom: 6, fontWeight: 500 }}>UPLOAD FILE</div>
          <label style={{ display: "block", marginBottom: 10 }}>
            ...
            <div style={{ background: isProcessing ? C.border : C.bg, border: `1px solid ${isProcessing ? C.gold : uploadedName ? C.green : C.border}`, color: isProcessing ? C.gold : uploadedName ? C.green : C.textSecondary, padding: "10px 14px", fontFamily: mono, fontSize: 15, cursor: isProcessing ? "default" : "pointer", borderRadius: 4 }}>
              {isProcessing ? "READING FILE..." : uploadedName ? `✓ ${uploadedName}` : "CHOOSE FILE →"}
            </div>
          </label>
          <div style={{ fontSize: 15, color: C.textMuted, fontFamily: mono, letterSpacing: "0.1em", marginBottom: 6, fontWeight: 500 }}>OR PASTE TEXT</div>
          <textarea value={canonUpload.content} onChange={e => setCanonUpload(p => ({ ...p, content: e.target.value }))}
            ...
            style={{ ...inputBase, fontSize: 15, resize: "vertical", marginBottom: 8 }} />
          <button onClick={uploadCanon} disabled={isUploading || !canonUpload.title || !canonUpload.content}
            style={{ width: "100%", background: C.gold, border: "none", color: C.bg, padding: "10px", fontFamily: sans, fontSize: 15, fontWeight: 500, letterSpacing: "0.05em", cursor: "pointer", borderRadius: 4 }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
              ...
              <div style={{ display: "flex", gap: 6 }}>
                <button onClick={() => toggleCanon(activeDoc.id, activeDoc.is_active)}
                  style={{ fontSize: 15, color: activeDoc.is_active ? C.textMuted : C.green, background: "transparent", border: `1px solid ${C.border}`, padding: "4px 10px", fontFamily: mono, cursor: "pointer", borderRadius: 4 }}>
                  {activeDoc.is_active ? "DEACTIVATE" : "ACTIVATE"}
                </button>
                <button onClick={() => deleteCanon(activeDoc.id)}
                  style={{ fontSize: 15, color: C.red, background: "transparent", border: `1px solid ${C.border}`, padding: "4px 10px", fontFamily: mono, cursor: "pointer", borderRadius: 4 }}>
                  DELETE
                </button>
              </div>
            </div>
            <div style={{ fontSize: 15, color: C.textMuted, fontFamily: mono, marginBottom: 32 }}>{activeDoc.content?.length?.toLocaleString()} chars · {activeDoc.is_active ? "active" : "inactive"}</div>
            <div style={{ fontSize: 15, color: C.textSecondary, lineHeight: 1.7, whiteSpace: "pre-wrap", fontFamily: sans }}><Highlight text={activeDoc.content} term={searchHighlight} /></div>
          </div>
        )
      }
    </div>
        ...
        <div style={{ maxWidth: 700 }}>
          <div style={{ marginBottom: 24 }}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
              <span style={{ fontSize: 15, color: C.textSecondary }}>{pending.length} open · {completed.length} complete</span>
              <span style={{ fontSize: 15, color: C.gold, fontFamily: mono }}>{pct}%</span>
            </div>
          ...
          <div style={{ display: "flex", gap: 6, marginBottom: 24 }}>
            {[{ id: "focus", label: "Next Up" }, { id: "workshops", label: "Workshops" }, { id: "all", label: "All Open" }].map(t => (
              <button key={t.id} onClick={() => setActionsView(t.id)}
                style={{ background: actionsView === t.id ? C.gold + "20" : "transparent", border: `1px solid ${actionsView === t.id ? C.gold + "60" : C.border}`, color: actionsView === t.id ? C.gold : C.textMuted, padding: "5px 12px", fontSize: 15, fontFamily: sans, fontWeight: 500, cursor: "pointer", borderRadius: 4 }}>
                {t.label}
              </button>
            ))}
          </div>
          ...
          {actionsView === "focus" && (
            <div>
              <div style={{ fontSize: 15, color: C.gold, fontFamily: mono, letterSpacing: "0.1em", marginBottom: 12 }}>YOUR NEXT 5 ACTIONS</div>
              {next5.length === 0
                ? <div style={{ color: C.textDisabled, fontStyle: "italic", fontSize: 15 }}>All caught up.</div>
                : next5.map(d => {
                    ...
                    return (
                      <div key={d.id} id={`del-${d.id}`}
                        ...
                        onMouseLeave={e => e.currentTarget.style.borderColor = C.border}>
                        <div style={{ fontSize: 15, color: C.textPrimary, lineHeight: 1.6, marginBottom: 6 }}><Highlight text={d.text} term={searchHighlight} /></div>
                        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                          <span style={{ fontSize: 15, color: cat.color, fontFamily: mono }}>{cat.icon} {cat.label}</span>
                          {d.idea?.text && <span style={{ fontSize: 15, color: C.textMuted, fontFamily: mono }}>· {d.idea.text.slice(0, 40)}...</span>}
                        </div>
                      </div>
                    );
                  })
              ...
              {byCategory.map(cat => (
                <div key={cat.id} style={{ marginBottom: 28, padding: "16px 18px", background: C.surface, border: `1px solid ${C.border}`, borderRadius: 8, borderTop: `3px solid ${cat.color}` }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
                    <span style={{ fontSize: 15, color: cat.color, fontFamily: mono, fontWeight: 500 }}>{cat.icon} {cat.label.toUpperCase()} WORKSHOP</span>
                    <span style={{ fontSize: 15, color: C.textMuted, fontFamily: mono }}>{cat.items.length} tasks</span>
                  </div>
                  {cat.items.slice(0, 5).map(d => (
                    <div key={d.id} id={`del-${d.id}`}
                      ...
                      <div style={{ fontSize: 15, color: C.textSecondary, lineHeight: 1.6 }}><Highlight text={d.text} term={searchHighlight} /></div>
                    </div>
                  ))}
                  {cat.items.length > 5 && <div style={{ fontSize: 15, color: C.textMuted, fontFamily: mono, marginTop: 8 }}>+{cat.items.length - 5} more</div>}
                </div>
              ))}
            </div>
          )}
          ...
          {actionsView === "all" && (
            pending.length === 0
              ? <div style={{ color: C.textDisabled, fontStyle: "italic", fontSize: 15 }}>All invitations complete.</div>
              : byCategory.map(cat => (
                  <div key={cat.id} style={{ marginBottom: 28 }}>
                    <div style={{ fontSize: 15, color: cat.color, fontFamily: mono, letterSpacing: "0.1em", marginBottom: 10 }}>{cat.icon} {cat.label.toUpperCase()}</div>
                    {cat.items.map(d => (
                      <div key={d.id} id={`del-${d.id}`}
                        onClick={() => toggleDeliverable(d.id, d.is_complete)}
                        ...
                        <div style={{ flex: 1 }}>
                          <div style={{ fontSize: 15, color: C.textSecondary, lineHeight: 1.6 }}><Highlight text={d.text} term={searchHighlight} /></div>
                          {d.idea?.text && <div style={{ fontSize: 15, color: C.textMuted, fontFamily: mono, marginTop: 4 }}>from: {d.idea.text.slice(0, 60)}...</div>}
                        </div>
                      </div>
                    ))}
                  </div>
    ...
    return (
      <div style={{ flex: 1, overflowY: "auto", padding: "36px 48px" }}>
        <div style={{ maxWidth: 700 }}>
          ...
          <div style={{ fontSize: 15, color: C.textMuted, marginBottom: 28 }}>Simple to-dos. No AI analysis.</div>
          <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 8, padding: 18, marginBottom: 32 }}>
            ...
            <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
              <input value={newTaskDue} onChange={e => setNewTaskDue(e.target.value)}
                ...
                style={{ flex: 1, background: C.surfaceHigh, border: `1px solid ${C.border}`, borderRadius: 4, color: C.textPrimary, fontSize: 15, padding: "8px 12px", outline: "none", fontFamily: mono }} />
              <button onClick={addTask} disabled={!newTaskText.trim() || taskAdding}
                style={{ background: newTaskText.trim() ? C.gold : C.surfaceHigh, color: newTaskText.trim() ? C.bg : C.textMuted, border: "none", borderRadius: 4, padding: "8px 18px", fontSize: 15, fontWeight: 600, cursor: newTaskText.trim() ? "pointer" : "default", fontFamily: sans }}>
                {taskAdding ? "..." : "Add"}
              </button>
            </div>
          </div>
          {openTasks.length > 0 && (
            <div style={{ marginBottom: 28 }}>
              <div style={{ fontSize: 15, color: C.textMuted, fontFamily: mono, letterSpacing: "0.15em", fontWeight: 500, marginBottom: 14 }}>OPEN ({openTasks.length})</div>
              {openTasks.map(t => {
                ...
                return (
                  <div key={t.id} style={{ display: "flex", alignItems: "flex-start", gap: 14, background: C.surface, border: `1px solid ${C.border}`, borderRadius: 8, padding: 16, marginBottom: 6, cursor: "pointer" }}>
                    ...
                    <div style={{ flex: 1 }} onClick={() => toggleDeliverable(t.id, t.is_complete)}>
                      <div style={{ fontSize: 15, color: C.textSecondary, lineHeight: 1.6 }}>{t.text}</div>
                      {due && <div style={{ fontSize: 15, color: due.color, marginTop: 6, fontFamily: mono }}>Due {due.text}</div>}
                    </div>
                    <div onClick={() => deleteTask(t.id)} style={{ fontSize: 15, color: C.textDisabled, cursor: "pointer", padding: "4px 8px" }}
          ...
          {doneTasks.length > 0 && (
            <div>
              <div style={{ fontSize: 15, color: C.textMuted, fontFamily: mono, letterSpacing: "0.15em", fontWeight: 500, marginBottom: 14 }}>DONE ({doneTasks.length})</div>
              {doneTasks.slice(0, 15).map(t => (
                <div key={t.id} style={{ display: "flex", alignItems: "center", gap: 14, background: C.surface, border: `1px solid ${C.border}`, borderRadius: 8, padding: 16, marginBottom: 6, opacity: 0.4, cursor: "pointer" }}>
                  <div onClick={() => toggleDeliverable(t.id, t.is_complete)}
                    style={{ width: 18, height: 18, borderRadius: 4, border: `2px solid ${C.green}`, background: C.green + "25", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 15, color: C.green, flexShrink: 0 }}>✓</div>
                  <div style={{ flex: 1, fontSize: 15, color: C.textDisabled, textDecoration: "line-through" }} onClick={() => toggleDeliverable(t.id, t.is_complete)}>{t.text}</div>
                  <div onClick={() => deleteTask(t.id)} style={{ fontSize: 15, color: C.textDisabled, cursor: "pointer", padding: "4px 8px" }}>✕</div>
                </div>
              ))}
            </div>
          )}
          {tasks.length === 0 && (
            <div style={{ color: C.textDisabled, fontStyle: "italic", fontSize: 15, textAlign: "center", padding: "40px 0" }}>No tasks yet. Add one above.</div>
          )}
        </div>
      </div>
    );
        ...
        : (
          <div style={{ flex: 1, display: "flex", flexDirection: "column", padding: "36px 48px", overflow: "hidden" }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 12 }}>
              <span style={{ fontSize: 15, color: C.textMuted, fontFamily: mono }}>Auto-saves as you type</span>
              <button onClick={() => {
                const content = composeContentRef.current?.value || "";
                const title = composeTitleRef.current?.value || "Untitled";
                ...
                style={{ background: "transparent", border: `1px solid ${C.border}`, color: C.textMuted, padding: "6px 14px", fontFamily: mono, fontSize: 15, cursor: "pointer", borderRadius: 4 }}>
                SAVE NOW
              </button>
            </div>
          </div>
    ...
    return (
      <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
        <div style={{ padding: "10px 24px", borderBottom: `1px solid ${C.border}`, display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10 }}>
          <span style={{ fontSize: 15, color: C.textMuted, fontFamily: mono }}>{filteredNodes.length} ideas · {filteredConns.length} connections</span>
          <select value={mapFilter} onChange={e => { setMapFilter(e.target.value); setFocusedNode(null); }}
            style={{ background: C.bg, border: `1px solid ${C.border}`, color: C.textPrimary, padding: "4px 8px", fontSize: 15, fontFamily: sans, borderRadius: 4, outline: "none" }}>
          ...
          <button onClick={async () => {
            ...
            style={{ background: "transparent", border: `1px solid ${C.border}`, color: C.textMuted, padding: "5px 12px", fontFamily: mono, fontSize: 15, letterSpacing: "0.08em", cursor: "pointer", borderRadius: 4, flexShrink: 0 }}>
            MAP ALL
          </button>
        </div>
            ...
            return (
              <div style={{ position: "absolute", top: 12, left: 16, zIndex: 50, background: C.surface, border: `1px solid ${C.border}`, borderRadius: 8, padding: "14px 16px", maxWidth: 360, maxHeight: "80vh", overflowY: "auto" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                  <span style={{ fontSize: 15, color: cat.color, fontFamily: mono, fontWeight: 500 }}>{cat.icon} {cat.label.toUpperCase()}</span>
                  <span onClick={() => { setActiveIdea(fi); navGo("library"); }}
                    style={{ fontSize: 15, color: C.gold, fontFamily: mono, cursor: "pointer" }}>OPEN IN LIBRARY →</span>
                </div>
                <div style={{ fontSize: 15, color: C.textPrimary, lineHeight: 1.5, marginBottom: 12 }}>{fi.text.slice(0, 120)}{fi.text.length > 120 ? "..." : ""}</div>
                {fc.length > 0 && (
                  <div style={{ borderTop: `1px solid ${C.border}`, paddingTop: 10 }}>
                    <div style={{ fontSize: 15, color: C.textMuted, fontFamily: mono, marginBottom: 8 }}>TOP {fc.length} CONNECTION{fc.length > 1 ? "S" : ""}</div>
                    {fc.map((c, ci) => {
                      ...
                      return (
                        <div key={ci} style={{ padding: "8px 10px", marginBottom: 4, background: C.bg, borderRadius: 6, border: `1px solid ${C.borderSubtle}` }}>
                          {other && <div style={{ fontSize: 15, color: otherCat?.color || C.textMuted, fontFamily: mono, marginBottom: 3 }}>{otherCat?.icon} {other.text.slice(0, 50)}...</div>}
                          <div style={{ fontSize: 15, color: C.textSecondary, lineHeight: 1.45 }}>{(c.reason || "").slice(0, 100)}{(c.reason || "").length > 100 ? "..." : ""}</div>
                ...
                <div onClick={() => setFocusedNode(null)}
                  style={{ marginTop: 10, fontSize: 15, color: C.textMuted, fontFamily: mono, cursor: "pointer", textAlign: "center" }}>✕ CLOSE</div>
              </div>
            );
          })()}
              ...
              <div key={node.id}
                ...
                {isHovered && (
                  <div style={{
                    ...
                    <div style={{ fontSize: 15, color: node.color, fontFamily: mono, marginBottom: 6 }}>{node.icon} {node.category.toUpperCase()}</div>
                    <div style={{ fontSize: 15, color: C.textPrimary, lineHeight: 1.6, marginBottom: nodeConns.length ? 10 : 0 }}>{node.fullText.slice(0, 120)}{node.fullText.length > 120 ? "..." : ""}</div>
                    {nodeConns.length > 0 && (
                      <div style={{ borderTop: `1px solid ${C.border}`, paddingTop: 8 }}>
                        <div style={{ fontSize: 15, color: C.textMuted, fontFamily: mono, marginBottom: 6 }}>{nodeConns.length} CONNECTION{nodeConns.length > 1 ? "S" : ""}</div>
                        {nodeConns.slice(0, 3).map((c, ci) => (
                          <div key={ci} style={{ fontSize: 15, color: C.textSecondary, lineHeight: 1.5, marginBottom: 4 }}>
                            → {c.reason}
                          </div>
                        ))}
                      </div>
  ...
  return (
    <div style={{ display: "flex", height: "100vh", background: "#131316", color: C.textPrimary, overflow: "hidden", padding: "8px", gap: 6 }}>
      {notification && (
        <div style={{ position: "fixed", top: 20, left: "50%", transform: "translateX(-50%)", background: C.surfaceHigh, border: `1px solid ${notification.type === "success" ? C.green : notification.type === "error" ? C.red : C.border}`, color: C.textPrimary, padding: "10px 22px", fontFamily: mono, fontSize: 15, letterSpacing: "0.1em", zIndex: 1000, borderRadius: 8 }}>
          {notification.msg}
        </div>
      )}
      ...
      <div style={{ width: leftW, background: C.surface, display: "flex", flexDirection: "column", flexShrink: 0, overflow: "hidden", borderRadius: 12 }}>
        <div style={{ padding: "16px 16px 12px", borderBottom: `1px solid ${C.border}` }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
            <div style={{ cursor: "pointer" }} onClick={() => navGo("dashboard")}>
              ...
              <div style={{ fontSize: 15, color: C.textMuted, fontFamily: mono, letterSpacing: "0.15em", marginTop: 2 }}>{user.project_name?.toUpperCase()}</div>
            </div>
            <div style={{ display: "flex", gap: 4 }}>
              <button onClick={handleSignOut} style={{ background: "transparent", border: "none", color: C.textDisabled, fontSize: 15, cursor: "pointer", padding: 4, fontFamily: mono }} title="Sign out">⏻</button>
              <button onClick={() => setLeftW(prev => prev <= 60 ? 260 : 60)} style={{ background: "transparent", border: "none", color: C.textMuted, fontSize: 15, cursor: "pointer", padding: 4 }} title="Toggle panel">◧</button>
            </div>
          </div>
          {projects.length > 1 && (
            <select value={currentProject?.id || ""} onChange={e => switchProject(e.target.value)}
              style={{ width: "100%", background: C.bg, border: `1px solid ${C.border}`, color: C.textSecondary, padding: "6px 8px", fontSize: 15, fontFamily: mono, outline: "none", borderRadius: 4, marginBottom: 10, cursor: "pointer" }}>
          ...
          <div style={{ position: "relative" }}>
            <input
              ...
              style={{ width: "100%", background: C.bg, border: `1px solid ${C.border}`, color: C.textPrimary, padding: "7px 12px 7px 28px", fontFamily: mono, fontSize: 15, outline: "none", borderRadius: 4 }}
            />
            <span style={{ position: "absolute", left: 10, top: "50%", transform: "translateY(-50%)", fontSize: 15, color: C.textDisabled }}>⌕</span>
              ...
              <div style={{ position: "absolute", top: "100%", left: 0, right: 0, background: C.surface, border: `1px solid ${C.border}`, borderTop: "none", maxHeight: 320, overflowY: "auto", zIndex: 200, borderRadius: "0 0 4px 4px" }}>
                {globalResults.map((r, i) => (
                  <div key={i} onClick={() => {
                    ...
                    <span style={{ fontSize: 15, color: C.textSecondary, lineHeight: 1.5, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}><Highlight text={r.label} term={globalSearch} /></span>
                  </div>
                ))}
              </div>
            )}
            {globalSearch && globalResults.length === 0 && globalSearch.length >= 2 && (
              <div style={{ position: "absolute", top: "100%", left: 0, right: 0, background: C.surface, border: `1px solid ${C.border}`, borderTop: "none", padding: "10px 12px", zIndex: 200 }}>
                <span style={{ fontSize: 15, color: C.textDisabled, fontStyle: "italic" }}>No results for "{globalSearch}"</span>
              </div>
            )}
          </div>
        </div>
        ...
        {(view === "library") ? (
          <>
            <div style={{ padding: "8px 10px", borderBottom: `1px solid ${C.border}` }}>
              <input
                ...
                style={{ width: "100%", background: C.bg, border: `1px solid ${C.border}`, color: C.textPrimary, padding: "6px 10px", fontFamily: sans, fontSize: 15, outline: "none", borderRadius: 4 }}
              />
            </div>
            <div style={{ padding: "8px 10px", borderBottom: `1px solid ${C.border}`, display: "flex", gap: 4, flexWrap: "wrap" }}>
              <button onClick={() => setFilterCat(null)}
                style={{ background: !filterCat ? C.gold : "transparent", color: !filterCat ? C.bg : C.textMuted, border: `1px solid ${!filterCat ? C.gold : C.border}`, padding: "3px 8px", fontSize: 15, fontFamily: sans, fontWeight: 500, cursor: "pointer", borderRadius: 4 }}>
                ALL {ideas.length}
              </button>
              {CATEGORIES.filter(cat => ideas.some(i => i.category === cat.id)).map(cat => (
                <button key={cat.id} onClick={() => setFilterCat(cat.id === filterCat ? null : cat.id)}
                  style={{ background: filterCat === cat.id ? cat.color : "transparent", color: filterCat === cat.id ? C.bg : C.textMuted, border: `1px solid ${filterCat === cat.id ? cat.color : C.border}`, padding: "3px 8px", fontSize: 15, fontFamily: sans, fontWeight: 500, cursor: "pointer", borderRadius: 4 }}>
                  {cat.icon} {cat.label}
                </button>
              ))}
            </div>
            <div style={{ flex: 1, overflowY: "auto" }}>
              {filtered.length === 0
                ? <div style={{ padding: 20, color: C.textDisabled, fontStyle: "italic", fontSize: 15 }}>Nothing here yet.</div>
                : filtered.map(idea => {
                    const cat = getCat(idea.category);
                    ...
                    return (
                      <div key={idea.id} onClick={() => { setActiveIdea(idea); setSearchHighlight(""); }}
                        ...
                        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 3 }}>
                          <span style={{ fontSize: 15, color: cat.color, fontFamily: mono, fontWeight: 500 }}>{cat.icon} {cat.label}</span>
                          <span style={{ fontSize: 15, color: C.textDisabled, fontFamily: mono }}>{daysAgo === 0 ? "today" : `${daysAgo}d`}{idea.signal_strength >= 4 ? " ◈" : ""}</span>
                        </div>
                        <div style={{ fontSize: 15, color: isActive ? C.textPrimary : C.textSecondary, lineHeight: 1.5, display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" }}><Highlight text={idea.text} term={localSearch} /></div>
                        {idea.signal_strength >= 4 && <div style={{ fontSize: 15, color: C.gold, fontFamily: mono, marginTop: 4 }}>◈ HIGH SIGNAL</div>}
                      </div>
                    );
                  })
              }
          ...
          <>
            <div style={{ padding: "10px", borderBottom: `1px solid ${C.border}` }}>
              <button onClick={() => setShowUpload(!showUpload)}
                style={{ width: "100%", background: showUpload ? "transparent" : C.gold, color: showUpload ? C.textMuted : C.bg, border: showUpload ? `1px solid ${C.border}` : "none", padding: "8px", fontFamily: sans, fontSize: 15, fontWeight: 500, letterSpacing: "0.05em", cursor: "pointer", borderRadius: 4 }}>
            ...
            <div style={{ flex: 1, overflowY: "auto" }}>
              {canonDocs.map(doc => (
                <div key={doc.id} onClick={() => setActiveDoc(doc)}
                  ...
                  <span style={{ fontSize: 15, color: doc.is_active ? C.green : C.textDisabled, flexShrink: 0 }}>{doc.is_active ? "✓" : "○"}</span>
                  <div style={{ overflow: "hidden" }}>
                    <div style={{ fontSize: 15, color: doc.is_active ? C.textPrimary : C.textMuted, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{doc.title}</div>
                    <div style={{ fontSize: 15, color: C.textDisabled, fontFamily: mono }}>{doc.type || "reference"}</div>
                  </div>
                </div>
              ))}
              {canonDocs.length === 0 && <div style={{ padding: 20, color: C.textDisabled, fontStyle: "italic", fontSize: 15 }}>No sources yet.</div>}
            </div>
          </>
        ) : (view === "compose") ? (
          <>
            <div style={{ padding: "10px", borderBottom: `1px solid ${C.border}` }}>
              <button onClick={createComposeDoc}
                style={{ width: "100%", background: C.gold, border: "none", color: C.bg, padding: "8px", fontFamily: sans, fontSize: 15, fontWeight: 500, letterSpacing: "0.05em", cursor: "pointer", borderRadius: 4 }}>
                + NEW DOCUMENT
              </button>
            </div>
            <div style={{ flex: 1, overflowY: "auto" }}>
              {composeDocs.length === 0
                ? <div style={{ padding: 20, color: C.textDisabled, fontStyle: "italic", fontSize: 15 }}>No documents yet.</div>
                : composeDocs.map(doc => (
                    <div key={doc.id} onClick={() => setActiveCompose(doc)}
                      ...
                      <div style={{ fontSize: 15, color: activeCompose?.id === doc.id ? C.textPrimary : C.textSecondary, marginBottom: 3 }}>{doc.title || "Untitled"}</div>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                        <span style={{ fontSize: 15, color: C.textMuted, fontFamily: mono }}>{doc.content?.length || 0} chars</span>
                        <button onClick={e => { e.stopPropagation(); deleteComposeDoc(doc.id); }}
                          style={{ fontSize: 15, color: C.red, background: "transparent", border: `1px solid ${C.border}`, padding: "2px 6px", fontFamily: mono, cursor: "pointer", borderRadius: 3 }}>
                          DEL
                        </button>
                      </div>
                    </div>
          ...
          <>
            <div style={{ padding: "10px 10px 4px", fontSize: 15, color: C.textMuted, fontFamily: mono, letterSpacing: "0.1em", fontWeight: 500 }}>
            ...
            <div style={{ overflowY: "auto", padding: "0 4px" }}>
              ...
              {canonDocs.length === 0 && <div style={{ padding: 12, fontSize: 15, color: C.textDisabled, fontStyle: "italic" }}>No sources yet.</div>}

              <div style={{ padding: "14px 8px 4px", fontSize: 15, color: C.textMuted, fontFamily: mono, letterSpacing: "0.1em", fontWeight: 500 }}>
                RECENT IDEAS
              </div>
              {ideas.slice(0, 8).map(idea => {
                ...
                return (
                  <div key={idea.id} onClick={() => navGo("library", idea)}
                    ...
                    onMouseEnter={e => !isActive && (e.currentTarget.style.background = C.surfaceHigh)}
                    onMouseLeave={e => !isActive && (e.currentTarget.style.background = "transparent")}>
                    <div style={{ fontSize: 15, color: cat.color, fontFamily: mono }}>{cat.icon} {cat.label}</div>
                    <div style={{ fontSize: 15, color: isActive ? C.textPrimary : C.textSecondary, lineHeight: 1.4, display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" }}>{idea.text}</div>
                  </div>
                );
              })}
            </div>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", background: C.bg, borderRadius: 12 }}>
        <div style={{ padding: "10px 28px", borderBottom: `1px solid ${C.border}`, background: C.surface, flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <span style={{ fontSize: 15, color: C.textMuted, fontFamily: mono, letterSpacing: "0.15em" }}>
          ...
          <div style={{ display: "flex", gap: 6 }}>
            <span style={{ fontSize: 15, color: C.textDisabled, fontFamily: mono }}>{ideas.length} ideas</span>
            <span style={{ fontSize: 15, color: C.textDisabled }}>·</span>
            <span style={{ fontSize: 15, color: C.textDisabled, fontFamily: mono }}>{pending.length} open</span>
          </div>
        </div>
        <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
      ...
      <div style={{ width: rightW, background: C.surface, display: "flex", flexDirection: "column", flexShrink: 0, overflow: "hidden", borderRadius: 12 }}>
        <div style={{ padding: "12px 14px 14px", borderBottom: `1px solid ${C.border}` }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
            <span style={{ fontSize: 15, color: C.textPrimary, fontWeight: 500 }}>Studio</span>
            <button onClick={() => setRightW(prev => prev <= 60 ? 290 : 60)} style={{ background: "transparent", border: "none", color: C.textMuted, fontSize: 15, cursor: "pointer", padding: 4 }} title="Toggle panel">◨</button>
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6 }}>
            {[
              ...
              <button key={tool.label} onClick={tool.action}
                ...
                <span style={{ fontSize: 15, color: tool.color, flexShrink: 0 }}>{tool.icon}</span>
                <span style={{ fontSize: 15, color: C.textSecondary, flex: 1, textAlign: "left" }}>{tool.label}</span>
                <span style={{ fontSize: 15, color: C.textDisabled }}>›</span>
              </button>
            ))}
          </div>
        ...
        <div style={{ padding: "8px 14px 0", display: "flex", borderBottom: `1px solid ${C.border}` }}>
          {[{ id: "insight", label: "Insight" }, { id: "patterns", label: "Patterns" }, { id: "stats", label: "Stats" }].map(t => (
            <button key={t.id} onClick={() => setStudioTab(t.id)}
              style={{ background: "transparent", border: "none", borderBottom: studioTab === t.id ? `2px solid ${C.gold}` : "2px solid transparent", color: studioTab === t.id ? C.textPrimary : C.textMuted, padding: "6px 12px 8px", fontSize: 15, fontWeight: 500, cursor: "pointer" }}>
              {t.label}
            </button>
          ))}
        </div>
          ...
          {studioTab === "insight" && (
            studioLoading
              ? <div style={{ padding: "20px 0", textAlign: "center" }}>
                  <div style={{ fontSize: 15, color: C.gold, marginBottom: 8, animation: "pulse 1.5s infinite" }}>✦</div>
                  <div style={{ color: C.textMuted, fontSize: 15, lineHeight: 1.8 }}>Analyzing your project...</div>
                  <div style={{ color: C.textDisabled, fontSize: 15, marginTop: 4 }}>This can take 10-15 seconds</div>
                </div>
              : studio ? (
                <div>
                  <div style={{ marginBottom: 18 }}>
                    <div style={{ fontSize: 15, color: C.gold, fontFamily: mono, letterSpacing: "0.12em", marginBottom: 8 }}>PROVOCATION</div>
                    <div style={{ fontSize: 15, color: C.textPrimary, lineHeight: 1.65, borderLeft: `3px solid ${C.gold}`, paddingLeft: 10 }}>{studio.provocation}</div>
                    <ReplyBox section="provocation" compact />
                  </div>
                  {studio.blind_spot && (
                    <div style={{ marginBottom: 18 }}>
                      <div style={{ fontSize: 15, color: C.red, fontFamily: mono, letterSpacing: "0.12em", marginBottom: 8 }}>BLIND SPOT</div>
                      <div style={{ fontSize: 15, color: C.textSecondary, lineHeight: 1.75 }}>{studio.blind_spot}</div>
                      <ReplyBox section="blind_spot" compact />
                    </div>
                  )}
                  {studio.urgentIdea && (
                    <div style={{ marginBottom: 18 }}>
                      <div style={{ fontSize: 15, color: C.green, fontFamily: mono, letterSpacing: "0.12em", marginBottom: 8 }}>ACT ON THIS NOW</div>
                      <div style={{ fontSize: 15, color: C.textSecondary, lineHeight: 1.75, fontStyle: "italic" }}>{studio.urgentIdea}</div>
                      <ReplyBox section="urgent" compact />
                    </div>
                  )}
                  <button onClick={() => { studioFired.current = false; setStudio(null); runStudio(ideas, user); }}
                    style={{ width: "100%", background: "transparent", border: `1px solid ${C.border}`, color: C.textMuted, padding: "7px", fontFamily: mono, fontSize: 15, letterSpacing: "0.1em", cursor: "pointer", marginTop: 4, borderRadius: 4 }}>
                    REFRESH ↻
                  </button>
                </div>
              ) : ideas.length < 2
                ? <div style={{ fontSize: 15, color: C.textDisabled, fontStyle: "italic", lineHeight: 1.8 }}>Capture a few ideas to activate the Studio.</div>
                : <button onClick={() => runStudio(ideas, user)}
                    style={{ width: "100%", background: C.gold, border: "none", color: C.bg, padding: "10px", fontFamily: mono, fontSize: 15, letterSpacing: "0.1em", cursor: "pointer", borderRadius: 4 }}>
                    GENERATE INSIGHT →
                  </button>
          )}
          {studioTab === "patterns" && (
            <div>
              <div style={{ marginBottom: 18 }}>
                <div style={{ fontSize: 15, color: C.textMuted, fontFamily: mono, letterSpacing: "0.12em", marginBottom: 6 }}>LIBRARY AUDIT</div>
                <div style={{ fontSize: 15, color: C.textMuted, lineHeight: 1.7, marginBottom: 8 }}>AI removes duplicates and test entries.</div>
                <button onClick={auditLibrary} disabled={auditing}
                  style={{ width: "100%", background: "transparent", border: `1px solid ${C.red}`, color: C.red, padding: "8px", fontFamily: mono, fontSize: 15, letterSpacing: "0.1em", cursor: auditing ? "default" : "pointer", opacity: auditing ? 0.5 : 1, borderRadius: 4 }}>
                  {auditing ? "AUDITING..." : "AUDIT + CLEAN LIBRARY"}
                </button>
              </div>
              {studio?.pattern && (
                <div style={{ marginBottom: 18 }}>
                  <div style={{ fontSize: 15, color: C.purple, fontFamily: mono, letterSpacing: "0.12em", marginBottom: 8 }}>WHAT YOU KEEP CIRCLING</div>
                  <div style={{ fontSize: 15, color: C.textSecondary, lineHeight: 1.65, borderLeft: `3px solid ${C.purple}`, paddingLeft: 10 }}>{studio.pattern}</div>
                </div>
              )}
              {studio?.duplicates && studio.duplicates !== "null" && (
                <div style={{ marginBottom: 18 }}>
                  <div style={{ fontSize: 15, color: C.gold, fontFamily: mono, letterSpacing: "0.12em", marginBottom: 8 }}>ON REPETITION</div>
                  <div style={{ fontSize: 15, color: C.textSecondary, lineHeight: 1.65, borderLeft: `3px solid ${C.gold}`, paddingLeft: 10 }}>{studio.duplicates}</div>
                </div>
              )}
              {ideas.length > 0 && (
                <div>
                  <div style={{ fontSize: 15, color: C.textMuted, fontFamily: mono, letterSpacing: "0.12em", marginBottom: 10 }}>BY CATEGORY</div>
                  {CATEGORIES.map(cat => {
                    const count = ideas.filter(i => i.category === cat.id).length;
                    if (!count) return null;
                    return (
                      <div key={cat.id} onClick={() => { setFilterCat(cat.id); navGo("library"); }} style={{ marginBottom: 8, cursor: "pointer" }}>
                        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 2 }}>
                          <span style={{ fontSize: 15, color: C.textSecondary }}>{cat.icon} {cat.label}</span>
                          <span style={{ fontSize: 15, color: cat.color, fontFamily: mono }}>{count}</span>
                        </div>
            ...
            <div>
              {[
                ...
                <div key={s.label} onClick={() => navGo(s.dest)}
                  ...
                  onMouseEnter={e => e.currentTarget.style.background = C.surfaceHigh}
                  onMouseLeave={e => e.currentTarget.style.background = "transparent"}>
                  <span style={{ fontSize: 15, color: C.textSecondary }}>{s.label}</span>
