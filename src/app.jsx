...
If no meaningful connections exist, return {"connections": []}`,
    ...
    return (
      <div style={{ marginTop: 10 }}>
        ...
        <div style={{ display: "flex", gap: 6 }}>
          ...
          {draft.trim() && (
            <button onClick={send}
              style={{ background: C.blue, border: "none", color: C.bg, padding: "7px 12px", fontFamily: mono, fontSize: 11, cursor: "pointer", flexShrink: 0 }}>
              ↵
            </button>
          )}
        </div>
  ...
  const SectionHead = ({ label, onClick }) => (
    <div onClick={onClick}
      style={{ fontSize: 11, color: C.textMuted, fontFamily: mono, letterSpacing: "0.15em", cursor: onClick ? "pointer" : "default", display: "inline-flex", alignItems: "center", gap: 5, transition: "color 0.15s" }}
  ...
  if (!authUser) return (
    <div style={{ height: "100vh", background: C.bg, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: sans }}>
      <div style={{ width: 340, display: "flex", flexDirection: "column", gap: 20 }}>
        ...
        />
        {authError && (
          <div style={{ fontSize: 11, color: C.red, textAlign: "center", lineHeight: 1.5 }}>{authError}</div>
        )}
        <button
  ...
  if (onboarding) return (
    <div style={{ height: "100vh", background: C.bg, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: sans }}>
      <div style={{ width: 380, display: "flex", flexDirection: "column", gap: 20 }}>
        ...
        <div style={{ fontSize: 11, color: C.textSecondary, textAlign: "center", lineHeight: 1.6 }}>
                ...
                return (
                  <div style={{ maxWidth: 640 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 22 }}>
                      ...
                      {displayIdea.signal_strength >= 4 && <span style={{ fontSize: 11, color: C.gold, fontFamily: mono, border: `1px solid ${C.gold}40`, padding: "2px 10px" }}>HIGH SIGNAL</span>}
                      {searchHighlight && <span onClick={() => setSearchHighlight("")} style={{ fontSize: 11, color: C.gold, fontFamily: mono, border: `1px solid ${C.gold}40`, padding: "2px 10px", cursor: "pointer" }}>✕ CLEAR HIGHLIGHT</span>}
                      <span style={{ flex: 1 }} />
                    ...
                    <div style={{ fontSize: 11, color: C.textPrimary, lineHeight: 1.65, marginBottom: 28, fontFamily: sans }}><Highlight text={displayIdea.text} term={searchHighlight} /></div>
                    {displayIdea.inspiration_question && (
                      <div style={{ marginBottom: 32, padding: "16px 20px", background: C.surfaceHigh, borderLeft: `3px solid ${C.textMuted}` }}>
                        ...
                        <div style={{ fontSize: 11, color: C.textSecondary, lineHeight: 1.65, fontStyle: "italic" }}><Highlight text={displayIdea.inspiration_question} term={searchHighlight} /></div>
                      </div>
                    )}
                    {displayIdea.ai_note && (
                      <div style={{ marginBottom: 32 }}>
                        <div style={{ fontSize: 11, color: C.gold, fontFamily: mono, letterSpacing: "0.12em", marginBottom: 10 }}>DRAMATURGICAL ANALYSIS</div>
                        <div style={{ fontSize: 11, color: C.textSecondary, lineHeight: 1.65 }}><Highlight text={displayIdea.ai_note} term={searchHighlight} /></div>
                    ...
                    {displayIdea.canon_resonance && (
                      <div style={{ marginBottom: 32 }}>
                        ...
                        <div style={{ fontSize: 11, color: C.textSecondary, lineHeight: 1.65 }}><Highlight text={displayIdea.canon_resonance} term={searchHighlight} /></div>
        ...
        : (
          <div style={{ maxWidth: 680 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
              <div style={{ fontSize: 16, color: C.textPrimary, fontWeight: 500 }}><Highlight text={activeDoc.title} term={searchHighlight} /></div>
              <div style={{ display: "flex", gap: 6 }}>
            ...
            <div style={{ fontSize: 11, color: C.textSecondary, lineHeight: 1.7, whiteSpace: "pre-wrap", fontFamily: sans }}><Highlight text={activeDoc.content} term={searchHighlight} /></div>
          </div>
        )
      }
    </div>
                ...
                : next5.map(d => {
                    ...
                    return (
                      <div key={d.id} id={`del-${d.id}`}
                        ...
                        onMouseLeave={e => e.currentTarget.style.borderColor = C.border}>
                        <div style={{ fontSize: 11, color: C.textPrimary, lineHeight: 1.6, marginBottom: 6 }}><Highlight text={d.text} term={searchHighlight} /></div>
                ...
                <div key={cat.id} style={{ marginBottom: 28, padding: "16px 18px", background: C.surface, border: `1px solid ${C.border}`, borderRadius: 8, borderTop: `3px solid ${cat.color}` }}>
                  ...
                  {cat.items.slice(0, 5).map(d => (
                    <div key={d.id} id={`del-${d.id}`}
                      ...
                      <div style={{ fontSize: 11, color: C.textSecondary, lineHeight: 1.6 }}><Highlight text={d.text} term={searchHighlight} /></div>
                    </div>
                  ))}
              ...
              {openTasks.map(t => {
                ...
                return (
                  <div key={t.id} style={{ display: "flex", alignItems: "flex-start", gap: 14, background: C.surface, border: `1px solid ${C.border}`, borderRadius: 8, padding: 16, marginBottom: 6, cursor: "pointer" }}>
                    ...
                    <div onClick={() => deleteTask(t.id)} style={{ fontSize: 11, color: C.textDisabled, cursor: "pointer", padding: "4px 8px" }}
            ...
            <div>
              ...
              {doneTasks.slice(0, 15).map(t => (
                <div key={t.id} style={{ display: "flex", alignItems: "center", gap: 14, background: C.surface, border: `1px solid ${C.border}`, borderRadius: 8, padding: 16, marginBottom: 6, opacity: 0.4, cursor: "pointer" }}>
                  ...
                  <div style={{ flex: 1, fontSize: 11, color: C.textDisabled, textDecoration: "line-through" }} onClick={() => toggleDeliverable(t.id, t.is_complete)}>{t.text}</div>
                  <div onClick={() => deleteTask(t.id)} style={{ fontSize: 11, color: C.textDisabled, cursor: "pointer", padding: "4px 8px" }}>✕</div>
                </div>
              ))}
            </div>
          )}
  ...
  const ComposeView = () => (
    <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
      {!activeCompose
        ? <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", color: C.textDisabled, fontStyle: "italic", fontSize: 11 }}>Select or create a document.</div>
        : (
      ...
      <div style={{ width: leftW, background: C.surface, display: "flex", flexDirection: "column", flexShrink: 0, overflow: "hidden", borderRadius: 12 }}>
        ...
        <div style={{ padding: "12px 12px 6px", display: "flex", flexDirection: "column", gap: 4 }}>
          <button onClick={() => navGo("dashboard")}
            ...
            <span style={{ fontSize: 16, color: view === "dashboard" ? C.gold : C.textSecondary, flexShrink: 0 }}>◉</span>
            <span style={{ fontSize: 16, color: view === "dashboard" ? C.gold : C.textSecondary, flex: 1, textAlign: "left" }}>Overview</span>
        ...
        {(view === "library") ? (
                ...
                : filtered.map(idea => {
                    ...
                    return (
                      <div key={idea.id} onClick={() => { setActiveIdea(idea); setSearchHighlight(""); }}
                        ...
                        <div style={{ fontSize: 11, color: isActive ? C.textPrimary : C.textSecondary, lineHeight: 1.5, display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" }}><Highlight text={idea.text} term={localSearch} /></div>
          ...
          <>
            ...
            <div style={{ overflowY: "auto", padding: "0 4px" }}>
              ...
              <div style={{ padding: "14px 8px 4px", fontSize: 11, color: C.textMuted, fontFamily: mono, letterSpacing: "0.1em", fontWeight: 500 }}>
                RECENT IDEAS
              </div>
