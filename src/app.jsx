...
Respond ONLY in raw JSON:
{
  ...
  const auditLibrary = async () => {
    ...
    try {
      ...
      const result = await callAI(
        `Audit a creative idea database. Be aggressive. Identify: (1) near-duplicates — same core thought even if worded differently, keep only the best version; (2) test entries. When in doubt, treat as duplicate.

Return ONLY raw JSON:
{ "toDelete": ["id1","id2"], "summary": "one sentence: what removed and why" }
If nothing: { "toDelete": [], "summary": "No duplicates or test entries found." }`,
        `ALL IDEAS:\n${allIdeas}`,
        800
      );
