...
Respond ONLY with raw JSON:
{
  ...
  const DashboardView = () => (
    <div style={{ flex: 1, overflowY: "auto", overflowX: "hidden", padding: "44px 52px" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 12, marginBottom: 40 }}>
        {[
          { label: "Ideas Captured",   value: ideas.length,   color: C.gold,   sub: `${ideas.filter(i => Date.now() - new Date(i.created_at) < 7*864e5).length} this week`, dest: "library" },
          { label: "Open Invitations", value: pending.length, color: C.red,    sub: `${deliverables.filter(d => d.is_complete).length} completed`, dest: "deliverables" },
          { label: "High Signal",      value: ideas.filter(i => i.signal_strength >= 4).length, color: C.green, sub: "worth pursuing now", dest: "library" },
          { label: "Canon Active",     value: activeCanon.length, color: C.purple, sub: "conditioning AI", dest: "canon" },
        ].map(s => (
          <div key={s.label} onClick={() => navGo(s.dest)}
            style={{ background: C.surface, border: `1px solid ${C.border}`, padding: "20px 18px", cursor: "pointer" }}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24, marginBottom: 36 }}>
        ...
        <div>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
            <span onClick={() => navGo("library")} style={{ fontSize: 10, color: C.textMuted, fontFamily: mono, letterSpacing: "0.15em", cursor: "pointer" }}
              onMouseEnter={e => e.currentTarget.style.color = C.gold}
              onMouseLeave={e => e.currentTarget.style.color = C.textMuted}>
              RECENT CAPTURES →
            </span>
            <span onClick={() => navGo("capture")} style={{ fontSize: 11, color: C.gold, cursor: "pointer", fontFamily: mono }}>+ CAPTURE</span>
          </div>
        ...
        <div>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
            <span onClick={() => navGo("deliverables")} style={{ fontSize: 10, color: C.textMuted, fontFamily: mono, letterSpacing: "0.15em", cursor: "pointer" }}
              onMouseEnter={e => e.currentTarget.style.color = C.gold}
              onMouseLeave={e => e.currentTarget.style.color = C.textMuted}>
              OPEN INVITATIONS →
            </span>
            <span style={{ fontSize: 11, color: C.textMuted, fontFamily: mono }}>{pending.length} open</span>
          </div>
          <div style={{ background: C.surface, border: `1px solid ${C.border}` }}>
            {pending.length === 0
      ...
      {canonDocs.length > 0 && (
        <div style={{ borderTop: `1px solid ${C.border}`, paddingTop: 28 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
            <span onClick={() => navGo("canon")} style={{ fontSize: 10, color: C.textMuted, fontFamily: mono, letterSpacing: "0.15em", cursor: "pointer" }}
              onMouseEnter={e => e.currentTarget.style.color = C.gold}
              onMouseLeave={e => e.currentTarget.style.color = C.textMuted}>
              CANON LAYER →
            </span>
            <span onClick={() => { navGo("canon"); setShowUpload(true); }} style={{ fontSize: 11, color: C.gold, cursor: "pointer", fontFamily: mono }}>+ ADD</span>
          </div>
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
            {canonDocs.map(doc => (
              <div key={doc.id} onClick={() => { setActiveDoc(doc); navGo("canon"); }}
                style={{ background: C.surface, border: `1px solid ${doc.is_active ? C.green + "50" : C.border}`, padding: "10px 14px", display: "flex", gap: 8, alignItems: "center", cursor: "pointer" }}
                onMouseEnter={e => e.currentTarget.style.borderColor = C.gold}
  ...
  const LibraryView = () => {
    // Auto-select first idea if nothing selected
    const displayIdea = activeIdea || filtered[0] || null;

    return (
    <div style={{ display: "flex", flex: 1, overflow: "hidden" }}>

      {/* List panel — tighter, cleaner */}
      <div style={{ width: 300, borderRight: `1px solid ${C.border}`, overflowY: "auto", flexShrink: 0, background: C.bg }}>

        {/* Category filters */}
        <div style={{ padding: "12px 16px", borderBottom: `1px solid ${C.border}`, display: "flex", gap: 5, flexWrap: "wrap" }}>
          <button onClick={() => setFilterCat(null)}
            style={{ background: !filterCat ? C.gold : "transparent", color: !filterCat ? C.bg : C.textMuted, border: `1px solid ${!filterCat ? C.gold : C.border}`, padding: "4px 12px", fontSize: 10, fontFamily: mono, cursor: "pointer", letterSpacing: "0.08em" }}>
            ALL {ideas.length}
          </button>
          {CATEGORIES.filter(cat => ideas.some(i => i.category === cat.id)).map(cat => (
            <button key={cat.id} onClick={() => setFilterCat(cat.id === filterCat ? null : cat.id)}
              style={{ background: filterCat === cat.id ? cat.color : "transparent", color: filterCat === cat.id ? C.bg : cat.color, border: `1px solid ${filterCat === cat.id ? cat.color : C.borderSubtle}`, padding: "4px 10px", fontSize: 10, fontFamily: mono, cursor: "pointer" }}>
              {cat.icon}
            </button>
          ))}
        </div>
        ...
        {filtered.length === 0
          ? <div style={{ padding: 40, color: C.textDisabled, fontStyle: "italic", fontSize: 14 }}>Nothing here yet.</div>
          : filtered.map(idea => {
              const cat = getCat(idea.category);
              ...
              return (
                <div key={idea.id} onClick={() => setActiveIdea(idea)}
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                    ...
                    <span style={{ fontSize: 10, color: C.textDisabled, fontFamily: mono }}>{daysAgo === 0 ? "today" : `${daysAgo}d ago`}</span>
                  </div>
                  <div style={{ fontSize: 14, color: isActive ? C.textPrimary : C.textSecondary, lineHeight: 1.65, display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" }}>
                    {idea.text}
                  </div>
                  {idea.signal_strength >= 4 && (
                    <div style={{ fontSize: 10, color: C.gold, fontFamily: mono, marginTop: 6 }}>◈ HIGH SIGNAL</div>
                  )}
                </div>
              );
            })
      ...
      {/* Detail panel — generous, readable */}
      <div style={{ flex: 1, overflowY: "auto", padding: "48px 56px", background: C.bg }}>
        {!displayIdea
          ? <div style={{ color: C.textDisabled, fontStyle: "italic", fontSize: 15 }}>No ideas yet.</div>
          : (() => {
              const cat = getCat(displayIdea.category);
              const ideaDeliverables = deliverables.filter(d => d.idea_id === displayIdea.id);
              return (
                <div style={{ maxWidth: 640 }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 24 }}>
                    ...
                    {displayIdea.signal_strength >= 4 && (
                      <span style={{ fontSize: 10, color: C.gold, fontFamily: mono, border: `1px solid ${C.gold}40`, padding: "3px 10px", letterSpacing: "0.1em" }}>HIGH SIGNAL</span>
                    )}
                  ...
                  {/* The idea itself — largest, most prominent */}
                  <div style={{ fontSize: 22, color: C.textPrimary, lineHeight: 1.85, marginBottom: 36, fontFamily: serif, fontWeight: "normal" }}>
                    {displayIdea.text}
                  </div>
                  ...
                  {displayIdea.inspiration_question && (
                    <div style={{ marginBottom: 32, padding: "16px 20px", background: C.surfaceHigh, borderLeft: `3px solid ${C.textMuted}` }}>
                      ...
                      <div style={{ fontSize: 15, color: C.textSecondary, lineHeight: 1.8, fontStyle: "italic" }}>{displayIdea.inspiration_question}</div>
                    </div>
                  )}
                    ...
                    <div style={{ marginBottom: 32 }}>
                      ...
                      <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                        {displayIdea.dimensions.map((d, i) => (
                          <span key={i} style={{ fontSize: 12, color: C.textSecondary, border: `1px solid ${C.border}`, padding: "5px 14px", fontFamily: mono }}>
                            {d.label}
                          </span>
                        ))}
                      </div>
                    ...
                    <div style={{ borderTop: `1px solid ${C.border}`, paddingTop: 28 }}>
                      ...
                      {ideaDeliverables.map(d => (
                        <div key={d.id} onClick={() => toggleDeliverable(d.id, d.is_complete)}
                          ...
                          onMouseEnter={e => e.currentTarget.style.background = C.surfaceHigh}
                          onMouseLeave={e => e.currentTarget.style.background = "transparent"}>
                          <div style={{ width: 18, height: 18, border: `2px solid ${d.is_complete ? C.green : C.border}`, background: d.is_complete ? C.green + "25" : "transparent", flexShrink: 0, marginTop: 3, transition: "all 0.15s" }} />
                          <div style={{ fontSize: 15, color: d.is_complete ? C.textDisabled : C.textSecondary, lineHeight: 1.75, textDecoration: d.is_complete ? "line-through" : "none" }}>
                            {d.text}
                          </div>
                        </div>
                      ))}
          ...
          <div>
            ...
            {activeCanon.length > 0 && (
              <div style={{ marginTop: 18 }}>
                <div onClick={() => navGo("canon")} style={{ fontSize: 10, color: C.textMuted, fontFamily: mono, letterSpacing: "0.12em", marginBottom: 10, cursor: "pointer" }}
                  onMouseEnter={e => e.currentTarget.style.color = C.gold}
                  onMouseLeave={e => e.currentTarget.style.color = C.textMuted}>
                  CANON ACTIVE →
                </div>
  ...
  return (
    <div style={{ display: "flex", height: "100vh", background: C.bg, color: C.textPrimary, overflow: "hidden" }}>
      ...
      <div style={{ width: 224, background: C.surface, borderRight: `1px solid ${C.border}`, display: "flex", flexDirection: "column", flexShrink: 0 }}>
        ...
        {/* Recent ideas in nav */}
        <div onClick={() => navGo("library")} style={{ padding: "14px 20px 6px", fontSize: 10, color: C.textMuted, fontFamily: mono, letterSpacing: "0.15em", cursor: "pointer" }}
          onMouseEnter={e => e.currentTarget.style.color = C.gold}
          onMouseLeave={e => e.currentTarget.style.color = C.textMuted}>
          RECENT →
        </div>
        ...
        {activeCanon.length > 0 && (
          <div style={{ padding: "12px 20px", borderTop: `1px solid ${C.border}` }}>
            <div onClick={() => navGo("canon")} style={{ fontSize: 10, color: C.textMuted, fontFamily: mono, letterSpacing: "0.12em", marginBottom: 6, cursor: "pointer" }}
              onMouseEnter={e => e.currentTarget.style.color = C.gold}
              onMouseLeave={e => e.currentTarget.style.color = C.textMuted}>
              CANON ACTIVE →
            </div>
            {activeCanon.slice(0, 2).map(d => (
              <div key={d.id} onClick={() => { setActiveDoc(d); navGo("canon"); }}
                style={{ fontSize: 11, color: C.green, marginBottom: 3, display: "flex", gap: 5, overflow: "hidden", cursor: "pointer" }}>
