...
export default function MindMapView({
  ...
  // Layout init: redo node positions whenever the idea/connection set changes.
  useEffect(() => {
  ...
  return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
      <div style={{ padding: "10px 24px", borderBottom: `1px solid ${C.border}`, display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10 }}>
        <span style={{ fontSize: 12, color: C.textMuted, fontFamily: mono }}>{filteredNodes.length} ideas · {filteredConns.length} connections</span>
        <select value={mapFilter} onChange={e => { setMapFilter(e.target.value); setFocusedNode(null); }}
          style={{ background: C.bg, border: `1px solid ${C.border}`, color: C.textPrimary, padding: "4px 8px", fontSize: 12, fontFamily: sans, borderRadius: 4, outline: "none" }}>
          ...
          return (
            <div style={{ position: "absolute", top: 12, left: 16, zIndex: 50, background: C.surface, border: `1px solid ${C.border}`, borderRadius: 8, padding: "14px 16px", maxWidth: 360, maxHeight: "80vh", overflowY: "auto" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                <span style={{ fontSize: 12, color: cat.color, fontFamily: mono, fontWeight: 500 }}>{cat.icon} {cat.label.toUpperCase()}</span>
                <span onClick={() => { onSetActiveIdea(fi); onNavigate("library"); }}
                  style={{ fontSize: 12, color: C.gold, fontFamily: mono, cursor: "pointer" }}>OPEN IN LIBRARY →</span>
              </div>
              <div style={{ fontSize: 12, color: C.textPrimary, lineHeight: 1.5, marginBottom: 12 }}>{fi.text.slice(0, 120)}{fi.text.length > 120 ? "..." : ""}</div>
              {fc.length > 0 && (
                <div style={{ borderTop: `1px solid ${C.border}`, paddingTop: 10 }}>
                  <div style={{ fontSize: 12, color: C.textMuted, fontFamily: mono, marginBottom: 8 }}>TOP {fc.length} CONNECTION{fc.length > 1 ? "S" : ""}</div>
                  {fc.map((c, ci) => {
                    ...
                    return (
                      <div key={ci} style={{ padding: "8px 10px", marginBottom: 4, background: C.bg, borderRadius: 6, border: `1px solid ${C.borderSubtle}` }}>
                        {other && <div style={{ fontSize: 12, color: otherCat?.color || C.textMuted, fontFamily: mono, marginBottom: 3 }}>{otherCat?.icon} {other.text.slice(0, 50)}...</div>}
                        <div style={{ fontSize: 12, color: C.textSecondary, lineHeight: 1.45 }}>{(c.reason || "").slice(0, 100)}{(c.reason || "").length > 100 ? "..." : ""}</div>
                        {c.strength >= 4 && <div style={{ fontSize: 12, color: C.gold, fontFamily: mono, marginTop: 3 }}>◈ STRONG</div>}
                      </div>
                    );
                  })}
                </div>
              ...
              <div onClick={() => setFocusedNode(null)}
                style={{ marginTop: 10, fontSize: 12, color: C.textMuted, fontFamily: mono, cursor: "pointer", textAlign: "center" }}>✕ CLOSE</div>
            </div>
          );
        })()}
            ...
            <div key={node.id}
              ...
              {isHovered && (
                <div style={{
                  ...
                  <div style={{ fontSize: 12, color: node.color, fontFamily: mono, marginBottom: 6 }}>{node.icon} {node.category.toUpperCase()}</div>
                  <div style={{ fontSize: 12, color: C.textPrimary, lineHeight: 1.6, marginBottom: nodeConns.length ? 10 : 0 }}>{node.fullText.slice(0, 120)}{node.fullText.length > 120 ? "..." : ""}</div>
                  {nodeConns.length > 0 && (
                    <div style={{ borderTop: `1px solid ${C.border}`, paddingTop: 8 }}>
                      <div style={{ fontSize: 12, color: C.textMuted, fontFamily: mono, marginBottom: 6 }}>{nodeConns.length} CONNECTION{nodeConns.length > 1 ? "S" : ""}</div>
                      {nodeConns.slice(0, 3).map((c, ci) => (
                        <div key={ci} style={{ fontSize: 12, color: C.textSecondary, lineHeight: 1.5, marginBottom: 4 }}>
                          → {c.reason}
                        </div>
                      ))}
                    </div>
