import { C, CATEGORIES, getCat, mono } from "../../lib/constants";
    ...
    <div style={{ flex: 1, overflowY: "auto", overflowX: "hidden", padding: "36px 44px" }}>
      ...
      <div style={{ marginBottom: 24, background: C.surface, border: `1px solid ${C.border}` }}>
        <div style={{ padding: "14px 18px", borderBottom: `1px solid ${C.border}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <span onClick={() => onNavigate("library")} style={{ fontSize: 12, color: C.gold, cursor: "pointer", fontFamily: mono }}>VIEW ALL →</span>
        </div>
        {ideas.length === 0
      ...
      <div style={{ marginBottom: 24, background: C.surface, border: `1px solid ${C.border}` }}>
        <div style={{ padding: "14px 18px", borderBottom: `1px solid ${C.border}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <span onClick={() => onNavigate("deliverables")} style={{ fontSize: 12, color: C.gold, cursor: "pointer", fontFamily: mono }}>VIEW ALL →</span>
        </div>
        {pending.length === 0
          ...
          : pending.slice(0, 5).map((task, idx, arr) => {
              ...
              return (
                <div key={task.id} onClick={() => onToggleDeliverable(task.id, task.is_complete)}
      ...
      {canonDocs.length > 0 && (
        <div style={{ background: C.surface, border: `1px solid ${C.border}`, padding: "16px 18px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
            ...
            <span onClick={() => onNavigate("canon")} style={{ fontSize: 12, color: C.gold, cursor: "pointer", fontFamily: mono }}>MANAGE →</span>
          </div>
